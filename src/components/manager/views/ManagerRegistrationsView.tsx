...
export const ManagerRegistrationsView = () => {
  const { events, isLoading, error, setViewing } = useManagerWorkspace();
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          Manage event registrations and export attendance data.
        </p>
      </div>

      <DashboardCard title="Event Registrations Overview" subtitle="Click on an event to view full details and export CSV.">
        <div className="divide-y divide-border/60">
            ...
            return (
              <div key={e.id} className="py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <Button variant="outline" size="sm" onClick={() => setViewing(e)} className="gap-1.5 border-border/60">
          ...
          {events.length === 0 && (
            <p className="text-sm text-muted-foreground py-8 text-center">No events yet. Create one to receive registrations.</p>
          )}
        </div>
      </DashboardCard>
    </div>
